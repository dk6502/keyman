...
void WaveThumbnail::paint(juce::Graphics &g) {
  ...
  if (waveform.getNumSamples() > 0) {
    ...
    for (int sample = 0; sample < audioPoints.size(); ++sample) {
      auto point =
          juce::jmap<float>(audioPoints[sample], -1.0f, 1.0f, height, 0);
    ...
    auto playHeadPosition =
        juce::jmap<int>(processor.getNumSamplerSounds(), 0,
    ...
    g.drawLine(playPosition, 0, playPosition, static_cast<float>(getHeight()),
               2.0f);
    ...
    g.setColour(juce::Colours::white);
    g.setFont(40.0f);
    g.drawFittedText("Drop an Audio File to Load", getLocalBounds(),
